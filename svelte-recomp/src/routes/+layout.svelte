...
/* The topbar is chrome, so it is pinned in px and does NOT follow --ui-scale.
   At the largest text size a scaled brand mark plus five 36px buttons is wider
   than a 360px phone, and what gets pushed off the edge is Sign out and the
   text-size control itself — i.e. you could make the text big enough that you
   could no longer make it small again. */
...
#pages{flex:1;overflow-y:auto;overflow-x:hidden;padding:18px 16px calc(var(--nav-h) + 28px + var(--sb));overscroll-behavior-y:contain}
  ...
  .ptr-inner{display:flex;align-items:center;gap:8px;font-size:0.71875rem;font-weight:700;color:var(--amber);white-space:nowrap}
